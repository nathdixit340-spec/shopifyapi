...
# Track active mass checks per user (to avoid duplicate polling)
...
def send_help(message):
    help_text = """
<b>🛒 Shopify Checker Bot (API Version)</b>

<b>Site Management:</b>
/seturl <code>example.com</code> – add a Shopify site
/urls – list your sites
/delurl <code>example.com</code> – remove a site

<b>Proxy Management:</b>
/setproxy <code>ip:port</code> or <code>user:pass@ip:port</code>
/proxies – list your proxies
/delproxy <code>ip:port</code> – remove a proxy
...
def poll_results(chat_id, user_id, task_id):
    while True:
        ...
        if status["status"] == "completed":
            ...
            # Preview first 15 cards
            for r in results[:15]:
                icon = "🔥" if "Charged" in r.get("response", "") else "✅" if r.get("success") else "❌"
                card_short = r['card'][:12] + "…" if len(r['card']) > 12 else r['card']
                msg += f"{icon} {card_short} → {r.get('response', '')[:40]}\n"
            if len(results) > 15:
...
def stop_mass(message):
    user_id = message.from_user.id
    if user_id in user_tasks:
        del user_tasks[user_id]  # Stop polling locally (API doesn't have a cancel endpoint)
        bot.reply_to(message, "⏹️ Mass check stop requested. (API will finish current checks, but bot will stop tracking.)")
    else:
        bot.reply_to(message, "❌ No active mass check to stop.")

# ========== RUN THE BOT ==========
if __name__ == "__main__":
    print("🤖 Shopify Checker Bot started. Waiting for commands...")
