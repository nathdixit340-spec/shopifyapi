...
API_TIMEOUT = 15          # Was 60
BIN_TIMEOUT = 15          # Was 60
PROXY_TIMEOUT = 8         # Was 12
RZ_TIMEOUT = 15           # Was 60

# General
BATCH_SIZE = 100          # Was 60
SITE_CHECK_BATCH = 60     # Was 40
HIT_DELAY = 1.5
PER_USER_LIMIT = 200
...
async def _execute_sp_check(uid, card, sites, proxies, plan, is_group, event):
    ...
    # Quick proxy test – keep only working ones (limit to 3 to save time)
    if proxies:
        working = []
        for p in proxies[:3]:  # Test only first 3 proxies to avoid delay
...
async def _execute_rz_check(uid, card, sites, proxies, plan, is_group, event):
    if uid not in ADMIN_ID and not is_paid_plan(plan):
        set_free_sp_last_use(uid)
        increment_free_sp_usage(uid)
    # Quick proxy test – same as Shopify
    if proxies:
        working = []
        for p in proxies[:3]:
            ok, _ = await test_proxy(p['proxy_url'])
