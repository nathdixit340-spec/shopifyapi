...
# APIs – UPDATE WITH YOUR RAILWAY API URL
SHOPIFY_API_URL = "https://your-railway-app.up.railway.app/shopify"
...
# Concurrent site testing for /addsites
async def test_shopify_site_concurrent(site, proxy_data=None, http_session=None, semaphore=None):
    async with semaphore:
        test_card = "5154623245618097|03|2032|156"
        try:
            result = await check_shopify_api(test_card, site, proxy_data, user_id=0, http_session=http_session)
            if result.get("Status") == "SiteError" or "captcha" in result.get("Response", "").lower():
...
# Modified /addsites with concurrent validation
...
async def add_shopify_sites_file(event):
    ...
    status_msg = await styled_reply(event, f"{PE} 🔄 {bs('Validating')} {len(sites)} {bs('sites concurrently...')}", emoji_ids=[CE["fire"]])
    
    semaphore = asyncio.Semaphore(10)
    http_session = await get_user_http_session(event.sender_id, "site")
    tasks = [test_shopify_site_concurrent(site, http_session=http_session, semaphore=semaphore) for site in sites]
    results = await asyncio.gather(*tasks)
    
    valid = [r['site'] for r in results if r['status'] == 'alive']
    invalid = [r['site'] for r in results if r['status'] == 'dead']
