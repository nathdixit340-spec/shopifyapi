...
def normalize_site_url(url):
    """For Shopify: keep only domain (no path)."""
    url = url.strip().lower()
    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                checked += 1
                lcd = card
                lrd = resp[:30]   # truncated for UI, full saved in hits
                if status == "Error":
                    errors += 1
                elif status == "Charged":
                    charged += 1
...
async def _send_mass_hit(card, result, status, uid, username, name, is_rz=False):
    ...
    try:
        ...
        resp = result.get('Response', '')   # FULL response, no truncation
        if is_rz:
...
async def send_final_file(uid, charged, approved, declined, errors, total, hits=None, target_chat=None):
    ...
    # Separate hits by status
    charged_list = []
    approved_list = []
    declined_list = []

    for hit in hits:
        # Expected format: "card - STATUS - response - gateway"
        parts = hit.split(" - ", 3)
        if len(parts) >= 3:
            card = parts[0]
            status = parts[1].upper()
            resp = parts[2].strip()
            gateway = parts[3] if len(parts) > 3 else "RazorPay"
        else:
            # fallback (should not happen)
            card = hit.split(" - ")[0] if " - " in hit else hit
            status = "UNKNOWN"
            resp = ""
            gateway = "RazorPay"

        if status == "CHARGED":
            charged_list.append((card, gateway, resp))
        elif status == "APPROVED":
            approved_list.append((card, gateway, resp))
        else:
            declined_list.append((card, gateway, resp))

    # Build file content
    lines = []
    lines.append("💳 RAZORPAY RESULTS 💳\n")
    lines.append("⚡\n")

    # Charged
    lines.append(f"✅ CHARGED ({len(charged_list)}):\n")
    for card, gw, resp in charged_list:
        lines.append(f"{card} | {gw} | ₹1 | {resp} | Unknown\n")
    lines.append("\n")

    # Approved
    lines.append(f"🔥 APPROVED ({len(approved_list)}):\n")
    for card, gw, resp in approved_list:
        lines.append(f"{card} | {gw} | ₹1 | {resp} | Unknown\n")
    lines.append("\n")

    # Declined
    lines.append(f"❌ DEAD ({len(declined_list)}):\n")
    for card, gw, resp in declined_list:
        lines.append(f"{card} | {gw} | ₹1 | {resp} | Unknown\n")
    ...
    try:
        ...
        log.error(f"Failed to send results file: {e}")
    finally:
        try:
            os.remove(fn)
        except:
...
async def single_cc_check_razorpay(event):
    ...
    try:
        ...
        msg, eid = format_rz_single_result(status, card, 'RazorPay', result.get('Response', ''), bi, elapsed)
        try: await lm.delete()
        except: pass
        rm2 = await styled_reply(event, msg, emoji_ids=eid, buttons=HIT_BUTTON)
        if status == "Charged":
...
async def view_proxies(event):
    ...
    for i, p in enumerate(proxies[:30], 1):
        # Show full proxy details (mask password)
        if p.get('username') and p.get('password'):
            pw = p['password']
            masked = pw[:2] + "*" * (len(pw)-4) + pw[-2:] if len(pw) > 4 else "****"
            display = f"{p['ip']}:{p['port']}:{p['username']}:{masked}"
        else:
            display = f"{p['ip']}:{p['port']}"
        text += f"<code>{i}.</code> {PE} <b>{display}</b>\n"
        eid.append(CE["link"])
