...
# APIs – UPDATE WITH YOUR RAILWAY URL
SHOPIFY_API_URL = "https://shopapi-production-f0cc.up.railway.app/shopify"
...
# Store pending proxy selection data
PENDING_PROXY_SELECTION = {}  # {user_id: {"type": "sp", "card": str, "sites": list, "plan": str, "is_group": bool, "event": event, "username": str, "name": str, "lm": message, "st": float}}
...
async def check_razorpay_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    ...
    for attempt in range(max_retries):
        ...
        if result.get("Status") != "RetryError":
            if rotator:
                if site: rotator.report_site_ok(site)
                if proxy_data: rotator.report_proxy_ok(proxy_data.get('proxy_url'))
            return result
        if rotator:
            if site: rotator.report_site_fail(site)
            if proxy_data and is_proxy_error(result.get("Response", "")):
                rotator.report_proxy_fail(proxy_data.get('proxy_url'))
        last = result
...
# ---------- /sp (Shopify Single) with proxy selection ----------
...
async def single_cc_check_shopify(event):
    ...
    if is_paid_plan(plan) or uid in ADMIN_ID:
        ...
        # free users: use admin's sites, no proxies
        sites = []
        for aid in ADMIN_ID:
    ...
    # If user has proxies and is premium, ask for preference
    if proxies and (is_paid_plan(plan) or uid in ADMIN_ID):
        ...
        try:
            sender = await event.get_sender()
            PENDING_PROXY_SELECTION[uid]["username"] = sender.username or f"user_{uid}"
            PENDING_PROXY_SELECTION[uid]["name"] = sender.first_name or "User"
        except:
            PENDING_PROXY_SELECTION[uid]["username"] = f"user_{uid}"
            PENDING_PROXY_SELECTION[uid]["name"] = "User"
    ...
    # No proxies or not premium – proceed without proxy
...
async def sp_proxy_cb(event):
    ...
    await _execute_sp_check(uid, data["card"], data["sites"], proxies, data["plan"], data["is_group"], data["event"], data["username"], data["name"])

async def _execute_sp_check(uid, card, sites, proxies, plan, is_group, event, username=None, name=None):
    # Apply free user daily limits
...
# ---------- /rz (Razorpay Single) with proxy selection ----------
...
async def single_cc_check_razorpay(event):
    ...
    # Determine sites and proxies for Razorpay
    if is_paid_plan(plan) or uid in ADMIN_ID:
        ...
        proxies = []
    if not sites:
        sites = [None]  # Allow no site (API will use its own)
...
async def rz_proxy_cb(event):
    ...
    await event.answer(f"{bs('Using')} {'proxy' if use_proxy else 'direct'}")
    proxies = data["proxies"] if use_proxy else []
    await _execute_rz_check(uid, data["card"], data["sites"], proxies, data["plan"], data["is_group"], data["event"], data["username"], data["name"])

async def _execute_rz_check(uid, card, sites, proxies, plan, is_group, event, username=None, name=None):
    if uid not in ADMIN_ID and not is_paid_plan(plan):
        set_free_sp_last_use(uid)
        increment_free_sp_usage(uid)
    if username is None or name is None:
...
# ---------- /msp (Shopify Mass) with proxy selection ----------
...
async def mass_check_shopify(event):
    ...
    # No proxies
    await _execute_msp_check(uid, cards, sites, [], event, from_inline)
...
# ---------- /mrz (Razorpay Mass) with proxy selection ----------
...
async def mass_check_razorpay(event):
    ...
    await _execute_mrz_check(uid, cards, sites, [], event, from_inline)
