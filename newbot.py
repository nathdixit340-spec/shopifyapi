...
async def add_shopify_sites_file(event):
    ...
    try:
        downloaded = await event.reply_to_message.document.download_media(bytes)
...
async def mass_check_shopify(event):
    ...
    if cmd_text: content = cmd_text; from_inline = True
        ...
        if rm.document:
            downloaded = await rm.document.download_media(bytes)
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if event.is_reply:
            ...
            if rm.file:
                downloaded = await rm.document.download_media(bytes)
                lines = [l.strip() for l in downloaded.decode().splitlines() if l.strip()]
