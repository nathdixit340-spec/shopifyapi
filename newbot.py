    ...
    async def worker(card):
        ...
        async with user_sem:
            ...
            try:
                ...
                checked += 1
                lcd = card
                lrd = resp[:30]   # keep truncated for UI only, full is sent in hit
                if status == "Error":
                    errors += 1
                elif status == "Charged":
                    charged += 1
...
async def single_cc_check_shopify(event):
    ...
    try:
        ...
        # Shopify single check: keep truncation as before (optional, but leave unchanged)
...
async def single_cc_check_razorpay(event):
    ...
    try:
        ...
        # FIX: no truncation - full response
...
async def view_proxies(event):
    ...
    for i, p in enumerate(proxies[:30], 1):
        # Show full proxy details (mask password partially)
        if p.get('username') and p.get('password'):
            masked_pw = p['password'][:2] + "*" * (len(p['password'])-4) + p['password'][-2:] if len(p['password']) > 4 else "****"
            display = f"{p['ip']}:{p['port']}:{p['username']}:{masked_pw}"
        else:
